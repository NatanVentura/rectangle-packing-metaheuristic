...
def generate_images(input_file):
    ...
    # Create the images
    create_image(n, blocks, positions, f"Without Genetic Algorithm - {input_file}")
    create_image(genetic_n, genetic_blocks, genetic_positions, f"With Genetic Algorithm - {input_file}")
```
```{python}
...
```{python}
# Generate images for each input file
for input_file in input_files:
    generate_images(input_file)
```
```{python}
